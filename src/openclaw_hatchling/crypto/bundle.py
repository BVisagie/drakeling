"""Encrypted .hatchling bundle format per Spec Section 17.
...
from openclaw_hatchling.crypto.identity import PRIVATE_KEY_FILENAME
from openclaw_hatchling.storage.database import DB_FILENAME
...
def export_bundle(data_dir: Path, passphrase: str) -> bytes:
    """Create an encrypted .hatchling bundle from the current creature data."""
...
def import_bundle(bundle: bytes, passphrase: str) -> tuple[bytes, bytes]:
    """Decrypt a .hatchling bundle and return (db_bytes, private_key_bytes)."""
