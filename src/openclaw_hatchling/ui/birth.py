...
class BirthScreen(Screen):
    ...
    BINDINGS: ClassVar[list[Binding]] = [
        Binding("r", "reroll", "Reroll colour", show=False),
    ...
    def on_mount(self) -> None:
        ...
        self.query_one("#action-label", Label).display = False
    ...
    def _update_egg_display(self) -> None:
        ...
        if self._colour_confirmed:
            ...
            reroll_label.update(
                f"Press R to try another colour  ({self._rerolls_remaining} remaining)"
            )
        else:
            reroll_label.update("No rerolls remaining")
    ...
    def action_confirm(self) -> None:
        if self._phase == "egg" and not self._colour_confirmed:
            ...
            self.query_one("#action-label", Label).display = True
    ...
    def _on_name_submitted(self, event: Input.Submitted) -> None:
        ...
        hex_tint = self._colour.hex_tint
        self.query_one("#action-label", Label).update(
            f"A [{hex_tint}]{self._colour.value}[/] Hatchling.\n"
            f"You will call them {self._name}.\n\n"
    ...
    async def _do_birth(self) -> None:
        try:
            ...
            self.query_one("#action-label", Label).update(f"Birth failed: {exc}")
            self._phase = "confirm"
            return

        self.query_one("#action-label", Label).update("Your Hatchling is here.")
        import asyncio
